...
.Lxts_enc:
	...
	vorr	$in4,$dat2,$dat2
	b.lo	.Lxts_enc_tail
	veor	$dat,$dat,$iv0				//before encryt, xor with iv
	veor	$dat2,$dat2,$iv1
	...
	vld1.8	{$dat4},[$inp],#16
	veor	$dat3,$dat3,$iv3			//  fourth block
	veor	$dat4,$dat4,$iv4
	sub	$len,$len,#32				// bias
	mov	$rounds,$rounds0
	b	.Loop5x_xts_enc
...
.Loop3x_xts_enc:
	...
	add	$inp,$inp,$xoffset			// $inp is adjusted in such way that
						// at exit from the loop $dat1-$dat2
						// are loaded with last "words"
...
.Lxts_enc_load_done:
	...
	// enrypt the composite block to get the last second encrypted text block
...
.Lxts_done:
	...
	// derypt the last secod block to get the last plain text block
.Lxts_dec_1st_done:
	eor	$tmpin,$dat0,$iv1
	ldr	$rounds,[$key1,#240]
	vld1.32	{$dat0},[$key1],#16
...
.Loop_final_2nd_dec:
	...
	// Compisite the tailcnt "16 byte not aligned block" into the last second plain blocks,
